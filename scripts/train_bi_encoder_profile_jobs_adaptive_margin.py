...
test evaluation protocol) EXCEPT the loss: instead of one fixed margin for
every triplet, the required anchor-positive vs. anchor-negative separation
scales with how far apart the positive and negative grades actually are.
...
own separate checkpoint/results paths so both experiments can be compared
side by side.
"""
...
def train_manual(model, triplets, rng, validation_data):
...
def main() -> None:
    ...
    best_state_dict, best_step, best_val_metrics, validation_curve = train_manual(
        model, triplets, train_rng, val_df
    )
